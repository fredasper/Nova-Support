...
async function refreshProfileFromBackend(email) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/appdib/backend/api/profile/get.php?email=${encodeURIComponent(email)}`);
...
async function loadFaqs() {
    try {
        const response = await fetch('http://localhost:8080/appdib/backend/api/faq/list.php');
        const data = await response.json();
